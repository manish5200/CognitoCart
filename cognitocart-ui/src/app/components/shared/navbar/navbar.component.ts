...
// ... (imports remain the same above, I am replacing from CartService down to menuOpen)
  ...
  template: `
    ...
    <div *ngIf="(auth.currentUser$ | async) as user">
      <div *ngIf="!user.emailVerified" class="verification-banner">
    ...
    <nav class="navbar" [class.has-banner]="((auth.currentUser$ | async)?.emailVerified === false) || ((auth.currentUser$ | async) && !(auth.currentUser$ | async)?.emailVerified)">
      <!-- Logo -->
      ...
      <!-- Right Actions -->
      <div class="nav-links">
        <ng-container *ngIf="!(auth.currentUser$ | async) && currentPath !== '/login' && !currentPath.startsWith('/register')">
...
export class NavbarComponent implements OnInit {
  ...
  search(): void {
    if (this.query.trim()) {
      this.router.navigate(['/products'], { queryParams: { aiq: this.query.trim() } });
    }
  }
